...
if (isServer) then {
    if (isMultiplayer) then {
		pca_disconnectHandle = (addMissionEventHandler ["HandleDisconnect", { params ["_unit"];
			if (isCopilotEnabled vehicle _unit) then {_unit action ["UnlockVehicleControl", vehicle _unit];};
			if (pca_disconnectAIUncon) then {_unit setVariable ["ace_medical_statemachine_AIUnconsciousness",true,true];};
		}]);
...
if (pca_resync_seatDisable) then {missionNamespace setVariable ["ace_medical_engine_disableSeatLocking", true];};
if (pca_resync_seatFix && {!pca_resync_seatDisable}) then {
    ...
    ["CAManBase", "GetOutMan", {
        ...
        if (local _vehicle || {_vehicle turretLocal _turret}) then {
            [_unit] call pca_resync_fnc_unlockUnconsciousSeat;
        };
    }] call CBA_fnc_addClassEventHandler;
    ...
    ["ace_unconscious", {
        ...
        if (!isNull _parent && { local _parent || {_parent turretLocal (_parent unitTurret _unit)}}) then {
    ...
    ["ace_killed", { // global event
        params ["_unit"];
        private _parent = (objectParent _unit);
        if (!isNull _parent && { local _parent || {_parent turretLocal (_parent unitTurret _unit)}}) exitWith {
            if (pca_resync_allowDumpDead) exitWith {
                [_unit] call pca_resync_fnc_unlockUnconsciousSeat;
            };
            [_unit] call pca_resync_fnc_lockUnconsciousSeat;
        };
